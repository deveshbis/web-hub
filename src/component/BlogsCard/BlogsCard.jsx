...
const BlogsCard = ({ blog, deletable,handleDelete }) => {
    ...
    return (
        <div className="flex relative">
            ...
            {deletable && (
            <div onClick={()=> handleDelete(id)} className="absolute bg-primary p-3 rounded-full hover:scale-105 -top-5 -right-5">
                <MdDeleteForever 
                size={20} 
                className="text-white group-hover:text-primary"></MdDeleteForever></div>
            )}
        </div>
    );
};
